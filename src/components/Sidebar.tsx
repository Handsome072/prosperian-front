...
import { ChevronDown, Filter, Users, Minus, MapPin } from 'lucide-react';
        ...
        <div className="p-4 bg-orange-50 border-t border-orange-200">
          ...
          <div className="space-y-1 text-xs text-orange-700">
            {filters.activities.length > 0 && (
              <div>• {filters.activities.length} activité(s)</div>
            )}
            {filters.cities.length > 0 && (
              <div>• {filters.cities.length} ville(s)</div>
            )}
            {filters.legalForms.length > 0 && (
              <div>• {filters.legalForms.length} forme(s) juridique(s)</div>
            )}
          </div>
        </div>
      )}
