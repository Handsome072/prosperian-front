    ...
    const loadData = async () => {
      try {
        ...
        if (type === 'subscription' && subscriptionId) {
          const response = await fetch(`http://69.62.105.91:4000/api/payment/subscriptions/${subscriptionId}`, {
            headers: {
          ...
          const response = await fetch(`http://69.62.105.91:4000/api/payment/credit-packs/${packId}`, {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
          });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (type === 'subscription' && subscriptionId) {
        ...
        const response = await fetch('http://69.62.105.91:4000/api/payment/create-subscription', {
          method: 'POST',
          headers: {
        ...
        const response = await fetch('http://69.62.105.91:4000/api/payment/create-payment-intent', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
